...
void main()
{
	...
	int p[2],i,j,pid;
	char str[10][10],temp[20]; // size of the string was not specified
	printf("\n");
	...
	else
	{
		...
		for(i=0;i<5;i++)
		{
			for(j=0;j<5;j++)
			{
				...
				/* BUG encountered and cleared : Array index out of bound, since we are trying to access element 
				j+1 which is not initialized (Remember there is no array index out of bound check in C , so we have to be
				vigilant while using such expressions in our program . So using 
				 appropriate size check j+1<5 such that it doesn't exceed the size*/
				
				{
				
				/* Here we are comparing the first non common occurring character of 				   
				both the strings and if the character of first string is having 
				 higher ASCII value as compared to the other, it returns a positive				    
